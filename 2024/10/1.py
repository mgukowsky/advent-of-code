...
def evaluate_trailhead(grid, coord, summits_reached):
    level = grid[coord[0]][coord[1]]

    if level == 9 and coord not in summits_reached:
        summits_reached.add(coord)
        return 1
    ...
    for move in MOVES:
        ...
        if (
            ...
            sum += evaluate_trailhead(grid, new_coord, summits_reached)
...
def sum_trailheads(input):
    ...
    for y, row in enumerate(grid):
        for x, val in enumerate(row):
            if val == 0:
                sum += evaluate_trailhead(grid, (y, x), set())
